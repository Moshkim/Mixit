...
@synthesize originalView = _originalView, labelWidth = _labelWidth, showNetworkActivityIndicator = _showNetworkActivityIndicator, tapEnabled = _tapEnabled;
...
{
    return [self newActivityViewForView:addToView withLabel:NSLocalizedString(@"Loading...", @"Default DSActivtyView label text") width:0 cancelTap:NO];
}
...
+ (DSActivityView *)newActivityViewForView:(UIView *)addToView withLabel:(NSString *)labelText cancelTap:(BOOL)tapEnabled;
{
    return [self newActivityViewForView:addToView withLabel:labelText width:0 cancelTap:tapEnabled];
}
...
+ (DSActivityView *)newActivityViewForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth cancelTap:(BOOL)tapEnabled;
{
    // Not autoreleased, as it is basically a singleton:
    return [[self alloc] initForView:addToView withLabel:labelText width:labelWidth cancelTap:tapEnabled];
}
...
- (DSActivityView *)initForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth cancelTap:(BOOL)tapEnabled;
{
...
- (void)tapActivityView:(UITapGestureRecognizer*)gesture {
    

    [[NSNotificationCenter defaultCenter] postNotificationName:GENERATING_VIDEO_CANCELLED object:nil];
}
...
+ (DSKeyboardActivityView *)newActivityViewWithLabel:(NSString *)labelText;
{
    ...
    if (!keyboardView)
        ...
        return (DSKeyboardActivityView *)[self newActivityViewForView:keyboardView withLabel:labelText cancelTap:NO];
}
